            ...
            if isRecording:
                if telemetry_line.find(',') != -1:
                    ...
                    if len(telemetry_data) > 9:
                        ...
                        if len(csv_lines) <= 0:
                            startTime = time
                        row = [len(csv_lines), '', '', float(telemetry_data[1]), float(telemetry_data[2]), float(telemetry_data[3]), float(
                            telemetry_data[5]), float(telemetry_data[6]), float(telemetry_data[7]), time - startTime]
    ...
    if not isRecording:
        ...
        if not isConnected:
            ...
            if len(csv_lines) > 0:
                now = datetime.now()
                filename = path.join(
                    battlelogs_folder, f"battlelog-" + now.strftime("%Y%m%d-%H%M%S") + ".csv")
...
def cast_spell_from_file(path):
    ...
    params = {'teamname': entry_team.get(), 'slot': int(
        entry_slot.get()), 'spellname': prediction[1], 'spellclass': prediction[0]}
    try:
        r = requests.get(url=duel_server_url, params=params)
        if r.status_code == 200:
            print("Spell successfully send to server")
        else:
            print("ERROR_URL: Spell could not be sent")
    except:
...
button_connect = tk.Button(
    master=frame_connect, text="Connect", command=connect)
button_connect.pack()
...
frame_slot = tk.Frame()
label_slot = tk.Label(
    master=frame_slot, text="Slot (1=left, 2=right):", width=15)
...
frame_cast = tk.Frame()
button_cast = tk.Button(
    master=frame_cast, text="Cast Spell", command=toggle_recording)
button_cast.pack()
...
button_test = tk.Button(
    master=frame_test, text="Test with Prerecording", command=test)
button_test.pack()
