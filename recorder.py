...
# configuration
# Wizard : Malvoris , Spell : Ignivarum
...
arduino_port = "/dev/cu.usbmodem11301"  # change this to your port value
            ...
            if isRecording:
                if telemetry_line.find(',') != -1:
                    ...
                    if len(telemetry_data) > 9:
                        ...
                        if len(csv_lines) <= 0:
                            startTime = time
                        row = [len(csv_lines), entry_wizard.get(), entry_spell.get(), float(telemetry_data[1]), float(telemetry_data[2]), float(
                            telemetry_data[3]), float(telemetry_data[5]), float(telemetry_data[6]), float(telemetry_data[7]), time - startTime]
...
def toggle_recording():
    ...
    if not isRecording:
        ...
        if not isConnected:
            ...
            if len(csv_lines) > 0:
                now = datetime.now()
                # change here for filename
                with open(path.join(recording_folder, f"test-" + now.strftime("%Y%m%d-%H%M%S") + ".csv"), "w", newline='') as csv_file:
...
button_record = tk.Button(
    master=frame_record, text="Record Spell", command=toggle_recording)
button_record.pack()
